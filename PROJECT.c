...
void addPatient() // f01 tiep nhan benh nhan
{
    ...
    while (1)
    {
        ...
        else if (strlen(patients[pCount].phone) < 9 || strlen(patients[pCount].phone) > 11)
        {
...
{
    int choice = 0;
    printf("Ban muon sap xep tang dan hay giam dan(1-tang dan/2-giam dan): \n");
    scanf("%d", &choice);
    if (choice == 1)
    {
    ...
    else
    {
        ...
        printf("Da sap xep theo cong no (giam dan)!\n");
    }
    const int pageSize = 5;
    int totalPages = (pCount + pageSize - 1) / pageSize;
    int currentPage = 1;
...
// ==============================
bool isDateDuplicate(const Record records[], int count, const char newDate[])
{
    for (int i = 0; i < count; i++)
    {
        if (strcmp(records[i].date, newDate) == 0)
        {
            return true;
        }
    }
...
void addRecord()
{
    ...
    do
    {
        ...
        if (strlen(records[rCount].recId) == 0)
        {
            printf("Ma lan kham rong! Vui long nhap lai.\n");
            continue;
        }
        break;
    } while (1);
    ...
    do
    {
        ...
        if (isDateDuplicate(records, rCount, records[rCount].date))
        {
            printf("Benh nhan %s da duoc ghi nhan vao ngay nay roi! Vui long nhap lai.\n", id);
            continue;
        }
        break;
    } while (1);
    ...
    do
    {
        ...
        if (strlen(records[rCount].status) == 0)
        {
            printf("Trang thai rong! Vui long nhap lai.\n");
            continue;
        }
        break;
    } while (1);
    ...
    while (scanf("%lf", &records[rCount].cost) != 1 || records[rCount].cost < 0)
    {
        ...
        printf("Gia tri khong hop le (phai la so >= 0).\n Nhap lai chi phi: ");

        if (records[rCount].cost == 0)
        {
            printf("Chi phi rong! Vui long nhap lai.\n");
            continue;
        }
        break;
    };
